...
export default function ContractorSignaturesPage() {
  ...
  const contractorDocs = allSignatureDocuments.filter((d) =>
    d.parties.some((p) => p.role === "Contractor")
  );

  const filtered = contractorDocs.filter((d) => {
  ...
  const statusCounts = {
    ...
    Draft: contractorDocs.filter((d) => d.status === "Draft").length,
    Sent: contractorDocs.filter((d) => d.status === "Sent").length,
    Viewed: contractorDocs.filter((d) => d.status === "Viewed").length,
    Signed: contractorDocs.filter((d) => d.status === "Signed").length,
    Completed: contractorDocs.filter((d) => d.status === "Completed").length,
  };

  const pendingMySign = contractorDocs.filter((d) =>
    d.status !== "Completed" && d.status !== "Draft" &&
    d.parties.some((p) => p.role === "Contractor" && !p.signed)
  ).length;
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 text-sm text-[#687068] mb-3">
            <Link href="/dashboard/contractor" className="hover:text-[#C28A78] transition-colors">Contractor Portal</Link>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-[#3A3F3A]">My Agreements</h1>
              <p className="text-sm text-[#687068] mt-1">Contracts and documents requiring your signature</p>
            </div>
          </div>
        </div>
        ...
        {pendingMySign > 0 && (
          <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/20 rounded-xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-[#3A3F3A]">{pendingMySign} agreement{pendingMySign > 1 ? "s" : ""} awaiting your signature</p>
              <p className="text-xs text-[#687068]">Please review and sign to continue working on jobs</p>
            </div>
            <button className="px-4 py-2 bg-[#F59E0B] text-white rounded-lg text-xs font-medium hover:bg-[#D97706] transition-colors whitespace-nowrap">
              View Pending
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "Total Agreements", value: contractorDocs.length, icon: "ri-file-list-3-line", color: "text-[#C28A78]", bg: "bg-[#C28A78]/10" },
            { label: "Pending My Sign", value: pendingMySign, icon: "ri-time-line", color: "text-[#F59E0B]", bg: "bg-[#F59E0B]/10" },
            { label: "Completed", value: contractorDocs.filter((d) => d.status === "Completed").length, icon: "ri-check-double-line", color: "text-[#10B981]", bg: "bg-[#10B981]/10" },
            { label: "Signed by Me", value: contractorDocs.filter((d) => d.parties.some((p) => p.role === "Contractor" && p.signed)).length, icon: "ri-pen-nib-line", color: "text-[#14B8A6]", bg: "bg-[#14B8A6]/10" },
          ].map((stat) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#E2E8F0] bg-[#F8FAFC]">
                  ...
                  <th className="text-left px-4 py-3 font-medium text-[#687068] hidden md:table-cell">My Status</th>
              ...
              <tbody className="divide-y divide-[#E2E8F0]">
                {filtered.map((doc) => {
                  const myParty = doc.parties.find((p) => p.role === "Contractor");
                  return (
                    <tr key={doc.id} className="hover:bg-[#F8FAFC] transition-colors">
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        {myParty?.signed ? (
                          <span className="text-xs font-medium text-[#10B981] bg-[#10B981]/10 px-2 py-0.5 rounded-full">Signed</span>
                        ) : (
                          <span className="text-xs font-medium text-[#F59E0B] bg-[#F59E0B]/10 px-2 py-0.5 rounded-full">Pending</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {myParty && !myParty.signed && doc.status !== "Draft" && (
                            <button
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center">
                      ...
                      <p className="text-sm text-[#94A3B8]">No agreements found</p>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                {selectedDoc.parties.some((p) => p.role === "Contractor" && !p.signed) && selectedDoc.status !== "Draft" && (
                ...
                <>
                  <div className="flex items-center gap-3 p-3 bg-[#F8FAFC] rounded-lg">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[#94A3B8]">Sign as: GreenPlumb Ltd (Contractor)</p>
                    </div>
                  </div>
                  <div>
