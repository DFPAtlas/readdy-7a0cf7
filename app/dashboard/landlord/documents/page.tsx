...
export default function LandlordDocumentsPage() {
  ...
  const landlordDocs = getDocumentsByRole("landlord").filter((doc) => {
    if (doc.propertyId === "all") return true;
    return landlordProperties.includes(doc.propertyId);
  });
  ...
  const filtered = landlordDocs.filter((d) => {
    ...
    const matchesProperty = propertyFilter === "all" || d.propertyId === propertyFilter || d.propertyId === "all";
    return matchesSearch && matchesFolder && matchesType && matchesProperty;
  });

  const folderCounts = documentFolders.reduce((acc, f) => {
    acc[f.id] = f.id === "all" ? landlordDocs.length : landlordDocs.filter((d) => d.folder === f.id).length;
    return acc;
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 text-sm text-[#687068] mb-3">
            ...
            <span className="text-[#3A3F3A] font-medium">Documents</span>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-[#3A3F3A]">Property Documents</h1>
              <p className="text-sm text-[#687068] mt-1">{filtered.length} documents across your portfolio</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 border border-[#E2E8F0] rounded-lg bg-white text-sm text-[#3A3F3A] whitespace-nowrap"
              >
                <span>{propertyFilter === "all" ? "All Properties" : propertiesForDocs.find(p => p.id === propertyFilter)?.name || propertyFilter}</span>
                <div className="w-4 h-4 flex items-center justify-center">
                  <i className="ri-arrow-down-s-line text-[#94A3B8] text-xs"></i>
                </div>
              </button>
              {showPropertyDropdown && (
                <div className="absolute top-full right-0 mt-1 bg-white border border-[#E2E8F0] rounded-lg shadow-lg z-30 min-w-[180px] max-h-[300px] overflow-y-auto">
                  {propertiesForDocs.map((p) => (
                    <button
                      key={p.id}
                      onClick={() => { setPropertyFilter(p.id); setShowPropertyDropdown(false); }}
                      className={`block w-full text-left px-3 py-2 text-sm hover:bg-[#F1F5F9] ${propertyFilter === p.id ? "text-[#C28A78] font-medium" : "text-[#3A3F3A]"}`}
                    >
                      {p.name}
                    </button>
                  ))}
