...
export default function TenantSignaturesPage() {
  ...
  const tenantDocs = allSignatureDocuments.filter((d) =>
    d.propertyId === tenantPropertyId &&
    d.parties.some((p) => p.role === "Tenant")
  );
  ...
  const statusCounts = {
    All: tenantDocs.length,
    Draft: tenantDocs.filter((d) => d.status === "Draft").length,
    Sent: tenantDocs.filter((d) => d.status === "Sent").length,
    Viewed: tenantDocs.filter((d) => d.status === "Viewed").length,
    Signed: tenantDocs.filter((d) => d.status === "Signed").length,
    Completed: tenantDocs.filter((d) => d.status === "Completed").length,
  };

  const pendingMySign = tenantDocs.filter((d) =>
    d.status !== "Completed" && d.status !== "Draft" &&
    d.parties.some((p) => p.role === "Tenant" && !p.signed)
  ).length;
    ...
    <DashboardShell>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[#3A3F3A]">My Signatures</h1>
            <p className="text-sm text-[#687068] mt-1">Documents requiring your signature</p>
          </div>
        </div>
        ...
        {pendingMySign > 0 && (
          <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/20 rounded-xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-[#3A3F3A]">{pendingMySign} document{pendingMySign > 1 ? "s" : ""} awaiting your signature</p>
              <p className="text-xs text-[#687068]">Please review and sign to complete the process</p>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "Total Documents", value: tenantDocs.length, icon: "ri-file-list-3-line", color: "text-[#C28A78]", bg: "bg-[#C28A78]/10" },
            { label: "Pending My Sign", value: pendingMySign, icon: "ri-time-line", color: "text-[#F59E0B]", bg: "bg-[#F59E0B]/10" },
            { label: "Completed", value: tenantDocs.filter((d) => d.status === "Completed").length, icon: "ri-check-double-line", color: "text-[#10B981]", bg: "bg-[#10B981]/10" },
            { label: "Signed by Me", value: tenantDocs.filter((d) => d.parties.some((p) => p.role === "Tenant" && p.signed)).length, icon: "ri-pen-nib-line", color: "text-[#14B8A6]", bg: "bg-[#14B8A6]/10" },
          ].map((stat) => (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#E2E8F0]">
                {filtered.map((doc) => {
                  const myParty = doc.parties.find((p) => p.role === "Tenant");
                  return (
          ...
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-5">
                  ...
                  {selectedDoc.parties.map((party) => (
                    <div key={party.id} className="flex items-center gap-3 px-3 py-2.5 bg-[#F8FAFC] rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {party.viewedAt && !party.signedAt && <p className="text-xs text-[#687068] mt-0.5">Viewed at {party.viewedAt}</p>}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                {selectedDoc.parties.some((p) => p.role === "Tenant" && !p.signed) && selectedDoc.status !== "Draft" && (
                ...
                <>
                  <div className="flex items-center gap-3 p-3 bg-[#F8FAFC] rounded-lg">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[#94A3B8]">Sign as: John Miller (Tenant)</p>
                    </div>
                  </div>
                  <div>
